package com.kanon.framework.shiro;
    ...
    public AuthenticationInfo doMultiRealmAuthentication(Collection<Realm> realms, AuthenticationToken token) throws AuthenticationException {
        ...
        while(var5.hasNext()) {
            ...
            if (realm.supports(token)) {
                ...
                if(token.getClass().getName().equals(com.kanon.framework.shiro.token.UsernamePasswordToken.class.getName())&&realm.getClass().getName().equals(NoPwdUserRealm.class.getName())){
                    try {
                        info = realm.getAuthenticationInfo(token);
                    } catch (AuthenticationException e) {
                        authenticationException = e;
