        ...
        try {
            ...
            try {
                ...
                if (StringUtils.isNotNull(user)) {
                    ...
                    // 记录用户退出日志
                    AsyncManager.me().execute(AsyncFactory.recordLoginInfo(userId, Constants.LOGOUT, MessageUtils.message("user.logout.success")));
                    // 清理缓存
                    SpringUtils.getBean(ISysUserOnlineService.class).removeUserCache(userId, ShiroUtils.getSessionId());
                }
                // 退出登录
                subject.logout();
