...
public class DbPersistenceDataService implements PersistenceDataService {
    ...
    public Integer save(PersistenceParam param) {
        ...
        Map<String, String> results = param.getSpiderResult() != null ? (Map<String, String>) param.getSpiderResult().getResults() : null;
        int re = 0;
        if (results != null) {
            ...
            for (java.util.Map.Entry<String, String> entry : insertData.entrySet()) {
                ...
                params.add(results.get(entry.getKey()) + "");
                value.add("?");
            }
    ...
    @Override
    public Integer savePage(PersistenceParam param) {
        String table = param.getTableName();
