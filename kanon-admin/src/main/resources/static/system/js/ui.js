...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            adds: function (id) {
                table.set();
                $.modal.open("批量添加" + table.options.modalName, $.operate.addsUrl(id));
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
                table.set();
            ...
            // 添加访问地址
            addsUrl: function (id) {
                var url = $.common.isEmpty(id) ? table.options.createUrls.replace("{id}", "") : table.options.createUrls.replace("{id}", id);
                return url;
            },
            // 修改信息
            edit: function (id) {
            ...
            successTabCallback: function (result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var $contentWindow = $('.System_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
